...
func findPlayer(players []*Player, playerKey string) (*Player, error) {
	...
	return nil, errors.New("could not find player with key")
}

// SelectRawStatistics returns a json string containing all the unprocessed statistics
func SelectRawStatistics(state *State) (string, error) {
	bytes, err := json.MarshalIndent(state.Statistics, "", "    ")
	if err != nil {
		return "", errors.New("error marshaling statistics into JSON")
	}
...
func SelectPlayerList(state *State) []*Player {
	players := []*Player{}
...
func SelectTeamList(state *State) []Team {
	teams := []Team{}
	rawStats := state.Statistics["statistic"].(map[string]interface{})
...
func SelectTeam(state *State, teamKey string) (Team, error) {
...
func SelectPlayerListForTeam(state *State, teamKey string) ([]Player, error) {
